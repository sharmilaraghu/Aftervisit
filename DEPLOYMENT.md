...
| `AFTER_VISIT_CONSOLE_PASSCODE` | The console's passcode, asked for on `/unlock` and checked by `proxy.ts` on every console page and server action — **required whenever the allowlist is open on a public URL** | The console is open to anyone; fine locally with calls locked |
