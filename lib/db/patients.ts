...
/**
 * Archive a patient. There is no hard delete.
 *
 * Three statements, in this order, each idempotent so a retry after a partial
 * failure converges:
 *
 *   1. skip every claimable call, so no window exists in which the patient is
 *      archived but the dialer still has work queued;
 *   2. close every live plan;
 *   3. stamp `archived_at`.
 *
...
export async function archivePatient(id: string): Promise<ArchiveResult> {
  ...
  const skipped = await db.execute(sql`
    update scheduled_calls
    set status = 'skipped', skip_reason = 'patient_archived', updated_at = now()
    where patient_id = ${id} and status in ('scheduled', 'claimed')
    returning id
  `);

  const closed = await db.execute(sql`
    update follow_up_plans
    set status = 'completed', closed_at = now(), close_reason = 'clinician_closed',
        updated_at = now()
    where patient_id = ${id} and status in ('active', 'paused', 'awaiting_approval')
    returning id
  `);
  ...
  return {
    archived: archived.rows.length > 0,
    callsSkipped: skipped.rows.length,
    plansClosed: closed.rows.length,
  };
}
